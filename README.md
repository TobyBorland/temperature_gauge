...
The characteristics of the thermistor used here was measured in water, where temperature and resistance are simultaneously checked with a thermometer and multimeter as the water cooled from boiling, higher values were obtained with a hairdryer and a multimeter K-type thermocouple. It may be easier to fit a polynomial to values taken at irregular temperature intervals to return estimated values at regular intervals. A spreadsheet calculates the predicted voltage $V_{temp}$, then maps this value to the 16-bit value between the supply voltage and zero (piecewise linear function lookup largely swiped from [Ben Shaya's thermistor library](https://github.com/beshaya/ArduinoThermistor/blob/master/thermistor.cpp)). As this build uses cheap components, the value of the fixed resistor and power supply voltage is measured.
...
<img src=./docs/images/board1.png alt= “” width=60% height=60%>
